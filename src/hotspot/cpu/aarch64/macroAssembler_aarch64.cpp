...
bool MacroAssembler::lookup_secondary_supers_table(Register r_sub_klass,
  ...
  // Make sure that result is nonzero if the TBZ below misses.
  mov(result, 1);
  ldrs(v_hash_slot, Address(r_super_klass, Klass::hash_slot_offset()));
  ldrd(v_bitmap, Address(r_sub_klass, Klass::bitmap_offset()));

  {
    ...
    // the bit is zero, we are certain that super_klass is not one of
    // the secondary supers.
    notr(vtemp3, T8B, v_hash_slot);
    assert(Klass::SECONDARY_SUPERS_TABLE_MASK <= 0xff, "must be");
    bici(vtemp3, T4S, (u1)(~ Klass::SECONDARY_SUPERS_TABLE_MASK));
    // vtemp3 == ~hash_slot & 63 == 63 - hash_slot
    ushl(vtemp3, T2D, vtemp1, vtemp3);
    fmovd(r_bitmap, vtemp3);
    tbz(r_bitmap, Klass::SECONDARY_SUPERS_TABLE_SIZE - 1, L_fallthrough);

    // Get the next array index that can contain super_klass into r_array_index.
    cnt(vtemp3, T8B, vtemp3);
    addv(vtemp3, T8B, vtemp3);
    fmovd(r_array_index, vtemp3);
    // NB! r_array_index is off by 1. It is compensated by keeping r_array_base off by 1 word.
    ...
    // call_VM_leaf_base(CAST_FROM_FN_PTR(address, poo), 0);

    // Is there another entry to check? Step to the next hash slot.
    movi(vtemp3, T16B, (u1)1);
    addv(v_hash_slot, T8B, v_hash_slot, vtemp3);

    // Check that the bitmap isn't full to bursting.
    fmovd(r_bitmap, v_bitmap);
    cmn(r_bitmap, (u1)1);
    br(NE, L_again);
  }

  { // Degenerate case: more than 63 secondary supers.
    ...
    // Use adds (not add) in order to reset the Z flag
    adds(r_array_base, r_array_base, Array<Klass*>::base_offset_in_bytes());
